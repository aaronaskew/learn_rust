...
/// Parse the string `s` as a coordinate pair, like `"400x600"` or `"1,2"`.
///
...
fn test_parse_complex() {
    assert_eq!(
        parse_complex("1.45,-0.045"),
        Some(Complex {
            re: 1.45,
            im: -0.045
        })
    );
    assert_eq!(parse_complex(",-0.45"), None);
}
...
///
/// The `bounds` argument gives gives the width and height of the buffer of `pixels`,
